...
$senha = 'u'.generatePassword(5);
$forekey     = $_SESSION['forekey'];

$sql = "UPDATE dependentes SET senha = '{$senha}' WHERE forekey = '{$forekey}'"; 
...
if($conexao->query($sql) === TRUE){
      ...
      >
        ...
        <tr style="height: 1rem; width: 100%; padding: 0 16px;">
          <td style="line-height: 1rem; font-size: .84rem; text-align: center;color: #606060; margin-left: 8px;">
            <span style="font-size: .79rem; text-transform: uppercase; font-weight: bold; color: #023bbf;font-family: Arial, Helvetica, sans-serif;">Código: </span>'.strip_tags($senha).'
          </td>
        </tr>
        <tr></tr>
      </table>
  ...
  try {
      ...
      $mail->Username   = 'unidentisdigital@unidentis.com.br';           // SMTP username
      $mail->Password   = 'vytjnokrszsgxaqy';                      // SMTP password
      $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;     // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
      $mail->Port       = 587;                                // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

      //Recipients
      $mail->setFrom('unidentisdigital@unidentis.com.br', 'Plano Unidentis');
